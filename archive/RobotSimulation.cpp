...
int RobotSimulation::cellsVisited(string program, int times) {
	...
	char *s;
	long siz;
	siz = times*program.sz;
	s = new char[siz];
	for(long i =0;i<times;i++)
	{
		for(int j = 0 ; j<program.sz; j ++)
		s[i*program.sz+j]=program[j];
	}
	long count=1;
	long x,y;
	vpii vis;
	...
	for(i=0;i<siz;i++){
		...
		tmp = mp(x,y);
		for(j =0 ; j <vis.sz ; j++){
			if(tmp.first == vis[j].first && tmp.second == vis[j].second){
				break;
			}
		}
		if(j >= vis.sz){
...
double test0() {
	string p0 = "RRR";
	int p1 = 100;
	RobotSimulation * obj = new RobotSimulation();
	clock_t start = clock();
	int my_answer = obj->cellsVisited(p0, p1);
	clock_t end = clock();
	delete obj;
